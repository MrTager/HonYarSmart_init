<template>
    <div class="date-selector">
        <div class="select-wrap" :style="{height:`${props.container_height*aspect_ratio}px`}">
            <ul class="select-options" ref="select_list" :style="{height:`${props.container_height*aspect_ratio}px`,transform:`translate3d(0, 0, ${-radius}px rotateX(0deg)`}" >
                <li class="select-option"  v-for="(item,i) in props.list" :key="i" :style="{height:`${props.item_height*aspect_ratio}px`,lineHeight:`${props.item_height*aspect_ratio}px`,top: `${props.item_height*aspect_ratio * -0.5}px`,'-webkit-transform': `rotateX(${-props.angle * i}deg) translate3d(0, 0, ${radius}px)`}">{{item}}</li>
            </ul>
            <!-- @touchstart="touchstart" @touchmove="touchmove" @touchend="touchend" -->
            <!-- ,transform:`translate3d(0, 0, ${-this.radius}px rotateX(0deg)` -->
            <div class="highlight">
...
export default {
    ...
    data(){
        return {
            ...
            event:{
                start_clientY:0,//触摸开始距离屏幕上边距
                start_scrollTop:0,//触摸开始的容器卷起高度
                move_direction:'up',//up、down
                last_move_position:0,//touchmove判断手指移动方向用
                acceleration:0,//手指移动加速度
                move_array:[],//用于计算加速度
            }
        }
    },
    methods:{
        ...
        //     console.log('触摸开始时间',new Date().getTime())
        //     _this.event.move_array = [[e.changedTouches[0].clientY,new Date().getTime()]]

        //     _this.event.start_clientY = e.changedTouches[0].clientY;
        //     _this.event.start_scrollTop = _this.$refs.select_list.scrollTop
        //     console.log('起始卷起高度',_this.event.start_scrollTop)
            
        //     _this.event.last_move_position = e.changedTouches[0].clientY;
        // },
        // touchmove(e){
        //     let _this = this;
        //     e = event || window.event
        ...
        //     //计算手指移动的方向
        //     if(_this.event.last_move_position < e.changedTouches[0].clientY){
        //         //向下
        //         this.event.move_direction = "down"
        //     }else{
        //         //向上
        //         this.event.move_direction = "up"
        //     }

        //     //计算加速度
        //     _this.event.move_array.push([e.changedTouches[0].clientY,new Date().getTime()])
        //     if(_this.event.move_array.length > 5){
        //         _this.event.move_array.shift();
        //     }

        //     //console.log('移动开始',_this.event.move_array);

        //     let move_clientY = e.changedTouches[0].clientY - _this.event.start_clientY;
        //     _this.$refs.select_list.scrollTop = _this.event.start_scrollTop - move_clientY
        //     console.log("移动中卷起高度",_this.event.start_scrollTop)
        //     console.log("计算高度",(_this.props.list.length-5)*(_this.props.container_height/(_this.props.count/2)))
        //     if(_this.event.move_direction === "up"){
        //         if(_this.event.start_scrollTop === (_this.props.list.length-5)*(_this.props.container_height/(_this.props.count/2))){
        //             _this.event.start_scrollTop = 0;
        //         }
        //     } else {
        //         if(_this.event.start_scrollTop === 0){
        //             _this.event.start_scrollTop = (_this.props.list.length-5)*(_this.props.container_height/(_this.props.count/2));
        //         }
        //     }
        // },
        // touchend(e){
        //     let _this = this;
        //     e = event || window.event
        //     // console.log('移动开始',e.changedTouches[0].clientY);
        //     // let move_clientY = e.changedTouches[0].clientY - _this.event.start_clientY;
        //     // _this.$refs.select_list.scrollTop -= move_clientY
        //     // console.log(move_clientY)

        //     let v;

        //     if (_this.event.move_array.length === 1) {
        //         v = 0;
        //     } else {
        //         let startTime = _this.event.move_array[_this.event.move_array.length - 2][1];
        //         let endTime = _this.event.move_array[_this.event.move_array.length - 1][1];
        //         let startY = _this.event.move_array[_this.event.move_array.length - 2][0];
        //         let endY = _this.event.move_array[_this.event.move_array.length - 1][0];

        //         // 计算速度
        //         v = ((startY - endY) / (_this.props.container_height*_this.aspect_ratio)) * 1000 / (endTime - startTime);
        //         let sign = v > 0 ? 1 : -1;

        //         v = Math.abs(v) > 30 ? 30 * sign : v;
        //         _this.event.acceleration = v
        //     }
        // },
    ...
    created(){
        ...
        /**圆环半径 */
        _this.radius = (_this.props.item_height * 3 / _this.props.count) / Math.tan((360 / _this.props.count) * Math.PI / 180)
        console.log("度数",_this.radius)

        /**形成列表 */
        let start_arr = new Array(24);
        for(let i = 0;i<start_arr.length;i++){
            start_arr[i] = i
        }
        _this.props.list = start_arr
        if(_this.mode === "loop"){
            // let head = start_arr.slice((start_arr.length-start_arr.length/8)+1,start_arr.length+1)
            // let foot = start_arr.slice(0,start_arr.length/8-1)
            // let end_arr = head.concat(start_arr).concat(foot)
            //_this.props.list = end_arr
