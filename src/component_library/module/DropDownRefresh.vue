<template>
    <div class="refresh-moudle"  @touchstart="touchStart()" @touchmove="touchMove()" @touchend="touchEnd()" :style="{transform: 'translate3d(0,' + top + 'px, 0)'}">
      <header class="pull-refresh">
        <slot name="pull-refresh">
          <div class="down-tip" v-if="dropDownState==1">
            <img v-if="dropDownInfo.downImg" class="down-img" :src="require('../../assets/images/drop/'+dropDownInfo.downImg)">
            <span class="down-text">{{dropDownInfo.downText}}</span>
          </div>
          <div class="up-tip" v-if="dropDownState==2">
            <img v-if="dropDownInfo.upImg" class="up-img" :src="require('../../assets/images/drop/'+dropDownInfo.upImg)">
            <span class="up-text">{{dropDownInfo.upText}}</span>
          </div>
          <div class="refresh-tip" v-if="dropDownState==3">
            <img v-if="dropDownInfo.refreshImg" class="refresh-img" :src="require('../../assets/images/drop/'+dropDownInfo.refreshImg)">
...
export default {
  ...
  data () {
    return {
      ...
      dropDownInfo: {
        downText: '下拉可以刷新',
        downImg: 'dropDown.png',
        upText: '松开立即刷新',
        upImg: 'dropDown.png',
        refreshText: '正在刷新数据...',
        refreshImg: 'dropLoading.png'
      },
      start_flag:false,
    }
  },
